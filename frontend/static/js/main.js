...
document.getElementById("form-estadocuenta").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!resp.ok || !data.ok) {
      mostrarAlerta("alerta-estadocuenta", data.mensaje || "No se pudo generar el estado de cuenta.");
      return;
    }

    const cont = document.getElementById("metricas-estadocuenta");
    cont.innerHTML = "";
    cont.appendChild(crearMetrica(data.contrato, "Contrato"));
    cont.appendChild(crearMetrica(data.pagos_encontrados, "Pagos encontrados", "verde"));
    cont.appendChild(crearMetrica(formatearMoneda(data.valor_contrato), "Valor del contrato"));
